...
def show(conn, c):
    ...
    with st.form("krovinio_forma", clear_on_submit=False):
        ...
        klientas          = col1.selectbox("Klientas", [""]+klientai, value=("" if is_new else cli["klientas"]))
        uzsakymo_numeris  = col2.text_input("Užsakymo numeris", value=("" if is_new else cli["uzsakymo_numeris"]))
        pakrovimo_numeris = col1.text_input("Pakrovimo numeris", value=("" if is_new else cli.get("pakrovimo_numeris","")))

        col3, col4 = st.columns(2)
        pak_data = col3.date_input("Pakrovimo data", value=(date.today() if is_new else pd.to_datetime(cli["pakrovimo_data"]).date()))
        pk_nuo   = col3.time_input("Laikas nuo", value=(time(8,0) if is_new else pd.to_datetime(cli["pakrovimo_laikas_nuo"]).time()))
        pk_iki   = col3.time_input("Laikas iki", value=(time(17,0) if is_new else pd.to_datetime(cli["pakrovimo_laikas_iki"]).time()))
        ...
        is_nuo   = col4.time_input("Laikas nuo", value=(time(8,0) if is_new else pd.to_datetime(cli["iskrovimo_laikas_nuo"]).time()))
        is_iki   = col4.time_input("Laikas iki", value=(time(17,0) if is_new else pd.to_datetime(cli["iskrovimo_laikas_iki"]).time()))

        col5, col6 = st.columns(2)
        pk_salis   = col5.text_input("Pakrovimo šalis", value=("" if is_new else cli.get("pakrovimo_salis","")))
        pk_miestas = col5.text_input("Pakrovimo miestas", value=("" if is_new else cli.get("pakrovimo_miestas","")))
        is_salis   = col6.text_input("Iškrovimo šalis", value=("" if is_new else cli.get("iskrovimo_salis","")))
        is_miestas = col6.text_input("Iškrovimo miestas", value=("" if is_new else cli.get("iskrovimo_miestas","")))

        col7, col8 = st.columns(2)
        vilkikas = col7.selectbox("Vilkikas", [""]+vilkikai_list, index=(0 if is_new else vilkikai_list.index(cli.get("vilkikas",""))+1))
        ...
        busena = st.selectbox("Būsena", busena_opt, index=(0 if is_new else busena_opt.index(cli.get("busena",""))))
    ...
    if submit:
        if pak_data > isk_data:
            ...
            conn.commit()
            st.success("Krovinys sėkmingai išsaugotas.")
            clear_selection()
