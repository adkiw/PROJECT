...
FIELD_ORDER = [
    ...
    "saskaitos_busena"
    # "svoris", "paleciu_skaicius" – pašalinta
]
...
def krovinys_to_pdf(data):
    ...
    # Viršuje – "ĮMONĖ X" ir antroje eilutėje "KROVINIO PERVEŽIMO UŽSAKYMAS"
    ...
    # Kiti laukai be vadybininkų ir limito
    fields = [
        ("Būsena", "busena"),
    ...
    # Išsaugome laikinai ir grąžiname kelią
...
def show(conn, c):
    ...
    # Užtikrinti, kad lentelėje yra tik tie stulpeliai, kurių reikia:
    expected = {
        ...
        'frachtas': 'REAL',
        'busena': 'TEXT'
        # Išimti: 'pakrovimo_miestas', 'pakrovimo_adresas', 'iskrovimo_miestas', 'iskrovimo_adresas', 'svoris', 'paleciu_skaicius'
    }
    ...
    # Pasirinktas krovinys atvaizdavimui/redagavimui
    if 'selected_cargo' not in st.session_state:
        st.session_state['selected_cargo'] = None
    if add_clicked:
        st.session_state['selected_cargo'] = 0  # 0 reiškia naujas
    def clear_sel():
        st.session_state['selected_cargo'] = None
    ...
    if sel is None:
        ...
        if df.empty:
            ...
            # Sukuriame du naujus lauko stulpelius „pakrovimo_vieta“ ir „iskrovimo_vieta“
            ...
            # Transporto ir atsakingas vadybininkas pagal „vilkikai“ lentelę
            ...
            # Apskaičiuojame „busena“
            busenos = []
            for _, row in df.iterrows():
                busenos.append(get_busena(c, row))
            df["busena"] = busenos

            # Rodome tik FIELD_ORDER stulpelius
            df_disp = df[FIELD_ORDER].fillna("")

            # Filtravimo eilutės virš stulpelių
    ...
    with st.form("cargo_form", clear_on_submit=False):
        ...
        eksped_vad = colA.selectbox("Eksp. vadybininkas", eksped_dropdown, index=eksped_idx, key="eksped_vad")
        ...
        if not is_new:
            try:
                ...
                pass
        pk_salis = colB.selectbox("Pakr. šalis", pk_salis_opts, index=pk_salis_index, key="pk_sal")
        pk_regionas = colB.text_input(
            "Pakr. regionas",
            value=("" if is_new else data.get('pakrovimo_regionas', '')),
            key="pk_regionas"
        )
        # Pašalinti: pk_mie, pk_adr
        ...
        if not is_new:
            try:
                ...
                pass
        is_salis = colC.selectbox("Iškr. šalis", is_salis_opts, index=is_salis_index, key="is_sal")
        is_regionas = colC.text_input(
            "Iškr. regionas",
            value=("" if is_new else data.get('iskrovimo_regionas', '')),
            key="is_regionas"
        )
        # Pašalinti: is_mie, is_adr
        ...
        sask_busena = colD.selectbox("Sąsk. būsena", sask_busenos, index=sask_busenos.index(sask_busena_val), key="sask_busena")
    ...
    if save:
        ...
        if pk_data > isk_data:
            ...
            vals = {
                ...
                # busena – nerašome ranka, ji skaičiuojama sąraše
            }
            try:
                if is_new:
                    cols = ",".join(vals.keys())
    ...
    # === PDF mygtukas (tik redagavimo režime, t.y. sel != None ir != 0) ===
    if sel not in (None, 0):
        ...
        if pdf_button:
            ...
            if not df_pdf.empty:
                ...
                # Atnaujiname laukus, kad sutaptų su FIELD_ORDER formatu:
