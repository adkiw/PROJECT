...
def human_header(name):
    # Automatiškai trumpina headerį į max 2 eilutes
    parts = name.split('_')
...
def show(conn, c):
    ...
    # Užtikrinam, kad papildomi stulpeliai yra
    ...
    # Norimų stulpelių tvarka (pagrindiniai priekyje)
    norimi = [
    ...
    if sel is None:
        ...
        if df.empty:
            ...
            for i, col in enumerate(df_disp.columns):
                hdr[i].markdown(f"<b>{human_header(col)}</b>", unsafe_allow_html=True)
